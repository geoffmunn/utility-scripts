...
class LiquidityTransaction(TransactionCore):
    ...
    def calcShareOutAmount(self, pool_id:int, coin:Coin) -> int:
        ...
        print ('getting pool details:', pool_id, coin.denom)
        ...
        total_share_exp = float(row[0] / shift_val)
        pool_asset_amount = int(row[1])

        # print ('token in amount:', token_in_amount)
        # print ('total share exp:', total_share_exp)
        # print ('pool asset amount:', pool_asset_amount)
        # print ('shift val:', shift_val)

        # print ((token_in_amount * total_share_exp))
        # print (((token_in_amount * total_share_exp) / pool_asset_amount))
        # print (((token_in_amount * total_share_exp) / pool_asset_amount) / shift_val)
    ...
    def simulate(self):
        ...
        ibc_value = sha256(f'transfer/{self.source_channel}/{self.liquidity_denom}'.encode('utf-8')).hexdigest().upper()
        liquidity_denom = 'ibc/' + ibc_value
        ...
        share_out_amount = share_out_amount * (1 - self.max_spread)
        ...
        #     share_out_min_amount = str(int(share_out_amount)),
        #     #token_in=Coin(denom=liquidity_denom, amount = int(self.liquidity_amount))
        #     token_in = token_in_coin
                
        # )

        # #test_fee:Fee = Fee(gas_limit="1", amount=Coins({Coin("ibc/0EF15DF2F02480ADE0BB6E85D9EBB5DAEA2836D3860E9F97F9AADE4F57A31AA0", 1)}))
        # #self.fee = test_fee
