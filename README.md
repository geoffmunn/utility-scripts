 ...
 * Swapping to a wide range of coins on Osmosis ([see list](docs/swap_options.md))
 * Joining and exiting Osmosis liquidity pools.
   * Only LUNC pools are currently supported
 * [Workflow support](docs/workflows.md) for everything in this list
...
> [!TIP]
> Installing terra-classic-sdk first should automatically install the terra-classic-proto dependency.
...
BASE swaps (undelegations) are also done in a batch at the end of the day, so you won't immediately see them in the undelegation list in the ```validators.py``` script. Try the next day and it should be there.
